...
public class JavaBeanTester {
	...
	public static <T> void test(final Class<T> clazz, final String... skipThese)
			throws IntrospectionException {
		final PropertyDescriptor[] props = Introspector.getBeanInfo(clazz)
				...
				if (params.length == 1 && params[0] == returnType) {
					...
					try {
						...
						assertEquals(String.format(
								"Failed while testing property %s",
								prop.getName()), expectedValue, actualValue);

					} catch (Exception ex) {
						fail(String
								.format("An exception was thrown while testing the property %s: %s",
	 ...
	 */
	public static <T> void findBooleanIsMethods(Class<T> clazz,
	...
	private static <T> void findTheReadMethod(PropertyDescriptor descriptor,
			Class<T> clazz) {
		try {
